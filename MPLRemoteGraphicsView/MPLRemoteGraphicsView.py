...
from pyqtgraph import CONFIG_OPTIONS
from pyqtgraph import multiprocess as mp

__all__ = ['MPLRemoteGraphicsView']
...
class MPLRemoteGraphicsView(QtWidgets.QWidget):
    """
...
class Renderer(QtWidgets.QWidget):
    ...
    def renderView(self):
        if self.img is None:
            ...
            # fn = os.path.join("c:", datetime.datetime.now().strftime("%H_%M_%S_%f.png"))
    ...
    def mousePressEvent(self, ev):
        # print("self._view.mousePressEvent(MouseEvent(ev), _callSync='off')")
        children = self.children()
        for c in children:
            if not isinstance(c, QtWidgets.QBoxLayout):
                c.mousePressEvent(ev)
        # ev.accept()
        return super().mousePressEvent(ev)

    def mouseReleaseEvent(self, ev):
        # print("self._view.mouseReleaseEvent(MouseEvent(ev), _callSync='off')")
        children = self.children()
        for c in children:
            if not isinstance(c, QtWidgets.QBoxLayout):
    ...
    def mouseMoveEvent(self, ev):
        # print("self._view.mouseMoveEvent(MouseEvent(ev), _callSync='off')")
        children = self.children()
        for c in children:
            if not isinstance(c, QtWidgets.QBoxLayout):
                c.mouseMoveEvent(ev)
    ...
    def wheelEvent(self, ev):
        # print("self._view.wheelEvent(WheelEvent(ev), _callSync='off')")
        # children = self.children()
        # for c in children:
        #     if not isinstance(c, QtWidgets.QBoxLayout):
        #         c.wheelEvent(ev)
    ...
    def enterEvent(self, ev):
        # print("self._view.enterEvent(EnterEvent(ev), _callSync='off')")
        # children = self.children()
        # for c in children:
        #     if not isinstance(c, QtWidgets.QBoxLayout):
        #         c.enterEvent(ev)
    ...
    def leaveEvent(self, ev):
        # print("self._view.leaveEvent(LeaveEvent(ev), _callSync='off')")
        # children = self.children()
        # for c in children:
        #     if not isinstance(c, QtWidgets.QBoxLayout):
        #         c.leaveEvents(ev)
